...
#define RF_INT               19     //  IN  Interrupt 2
#define RF_A                 14     //  IN  DVORISNA_RASVETA
#define RF_B                 15     //  IN  SVETLO_IS_GARAZA
#define RF_C                 16     //  IN  SVETLO_IS_KUCE
#define RF_D                 17     
#define RF_E                 18
...
char lux_Ch[5], potDimCh[5];

int potDim, potDimOld;

void rf_prijem() {     // Interapt rutina pri prijemu signala

 if ( digitalRead(RF_A) == 1 )   { client.publish("kuca/Dvo/sv1","ON" );  } //"kuca/Dvo/sv1" Dvorisna rasveta
 if ( digitalRead(RF_A) == 0 )   { client.publish("kuca/Dvo/sv1","OFF" ); } 

 if ( digitalRead(RF_B) == 1 )   { client.publish("kuca/Dvo/sv2","ON" );  } //"kuca/Dvo/sv2" Svetlo ispred garaze
 if ( digitalRead(RF_B) == 0 )   { client.publish("kuca/Dvo/sv2","OFF" ); }

 if ( digitalRead(RF_C) == 1 )   { client.publish("kuca/Dvo/sv3","ON" );  } //"kuca/Dvo/sv3" Svetlo ispred kuce
 if ( digitalRead(RF_C) == 0 )   { client.publish("kuca/Dvo/sv3","OFF" ); }
...
void setup()
{
    ...
    pinMode(RF_INT,INPUT_PULLUP);   // Interrupt 2
    attachInterrupt(digitalPinToInterrupt(RF_INT), rf_prijem, FALLING);
    
    pinMode(RF_A,INPUT);     // Dvorisna rasveta
    pinMode(RF_B,INPUT);     // Svetlo ispred garaze
    pinMode(RF_C,INPUT);     // Svetlo ispred kuce
    pinMode(RF_D,INPUT);
    pinMode(RF_E,INPUT);
...
// **** Salji stanje Dimera svetla na openHAB sa potenciometra A0 ULAZ *********
 ...
 if ( potDimOld != potDim ) {
  ...
  client.publish("kuca/Dvo/svd1", potDimCh); 
  }
 potDimOld = potDim;      
...
if (light.getData(data0,data1))
  ...
  dtostrf(lux,5,0,lux_Ch);
  Serial.print("Nivo dnevnog svetla je:  ");
  Serial.println(lux_Ch);
  client.publish("kuca/Dvo/svn1",lux_Ch); 
  }
